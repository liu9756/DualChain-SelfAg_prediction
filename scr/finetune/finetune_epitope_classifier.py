# -*- coding: utf-8 -*-
"""
Fine-tune EpitopeClassifier on small 500-sample dataset (featurized_ft)
using backbone weights from the large minT model.
python finetune_epitope_classifier.py \
  --data /users/PAS2177/liu9756/DualChain-SelfAg_prediction/data/featurized_ft \
  --pretrained /users/PAS2177/liu9756/DualChain-SelfAg_prediction/results/train_results_min10/best_acc.pt \
  --outdir /users/PAS2177/liu9756/DualChain-SelfAg_prediction/results/finetune_results_min10 \
  --epochs 300 --bs 64 --d_model 256 \
...
class TcrDataset(torch.utils.data.Dataset):
    def __init__(self, X, y, a_ids, b_ids, row_idx):
        ...
        assert len(self.row_idx) == n, f"row_idx length {len(self.row_idx)} != {n}"
...
# backbone model
def load_backbone_partial(model, ckpt_path, device):
    ckpt = torch.load(ckpt_path, map_location=device)
    sd_pre = ckpt["state_dict"]
    sd_new = model.state_dict()

    loaded, skipped = [], []
    for k, v in sd_new.items():
        if k in sd_pre and sd_pre[k].shape == v.shape:
            sd_new[k] = sd_pre[k]
            loaded.append(k)
        else:
            skipped.append(k)
    ...
    print(f"[FT init] loaded {len(loaded)} keys from backbone, skipped {len(skipped)}")
    # print("  loaded examples:", loaded[:10])
...
def run(args):
    ...
    X, y, a_ids, b_ids, meta = load_featurized(data_dir)
    num_classes = len(meta["label_info"]["labels_vocab"])
    x_dim = X.shape[1]
    meta["hla_vocab_size"] = int(meta.get("hla_vocab_size", X.shape[1]))

    print(f"[Data] X shape={X.shape}, #classes={num_classes}")

    idx_tr = np.load(data_dir / "idx_train.npy").astype(int)
    idx_val = np.load(data_dir / "idx_val.npy").astype(int)
    idx_te = np.load(data_dir / "idx_test.npy").astype(int)

    all_idx = np.concatenate([idx_tr, idx_val, idx_te])
    uniq_idx = np.unique(all_idx)
    assert len(uniq_idx) == X.shape[0], \
        f"Split indices do not cover all samples: uniq={len(uniq_idx)}, N={X.shape[0]}"
    ...
    a_tr, a_val, a_te = a_ids[idx_tr], a_ids[idx_val], a_ids[idx_te]
    b_tr, b_val, b_te = b_ids[idx_tr], b_ids[idx_val], b_ids[idx_te]

    ds_tr  = TcrDataset(X_tr,  y_tr,  a_tr,  b_tr,  idx_tr)
    ds_val = TcrDataset(X_val, y_val, a_val, b_val, idx_val)
    ds_te  = TcrDataset(X_te,  y_te,  a_te,  b_te,  idx_te)
    ...
    model = EpitopeClassifier(
        ...
        lambda_hla=args.lambda_hla,
        lambda_seq_alpha=args.lambda_seq,
        lambda_seq_beta=args.lambda_seq,
        d_model=args.d_model,
        use_hla=bool(args.use_hla),
        use_seq_a=bool(args.use_seq_a),
        use_seq_b=bool(args.use_seq_b),
        dropout_tab=args.dropout_tab,
        dropout_seq=args.dropout_seq,
        use_cosine_head=bool(args.use_cosine_head),
        margin_m=args.margin_m,
        scale_s=args.scale_s,
        slot_drop=args.slot_drop
    ).to(device)


    load_backbone_partial(model, args.pretrained, device)

    # AMP
    scaler = torch.cuda.amp.GradScaler(
        enabled=(torch.cuda.is_available() and bool(args.amp) and not args.cpu)
    )
    ...
    with open(out / "hparams_ft.json", "w") as f:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, aa, bb, yb, _ in dl_tr:
            ...
            with torch.amp.autocast("cuda", enabled=scaler.is_enabled()):
                logits = model(
                    xb, aa, bb,
                    y=yb if model.use_cosine_head else None,
        ...
        print(f"[Epoch {epoch:03d}] train_loss={loss_sum/len(ds_tr):.4f}  "
              f"val_loss={val_loss/len(ds_val):.4f}  ACC={acc:.3f}  F1m={f1m:.3f}  "
              f"Top5={top5:.3f}  Top1={top1:.3f}")
    ...
    with torch.no_grad():
        for xb, aa, bb, yb, ridx in dl_te:
            ...
            top1_pred.extend(p1.tolist())
            top1_prob.extend(s1.tolist())
            top5_pred.extend(topk_idx.tolist())
            top5_prob.extend(topk_p.tolist())
    ...
    print(f"\n=== TEST (FT) ===  ACC={acc:.3f}  F1m={f1m:.3f}  Top5={top5:.3f}  Top10={top10:.3f}")
    ...
    if best_state is not None:
        ...
        print(f"[Best FT Model Saved] {best_path} (includes temperature).")

    csv_path = out / "test_predictions_ft.csv"
        ...
        for i in range(len(rows)):
            w.writerow(
                [
                    ...
                    int(top1_pred[i]),
                    float(top1_prob[i]),
                    ";".join(map(str, top5_pred[i])),
                    ";".join(f"{x:.4f}" for x in top5_prob[i]),
                ]
            )
...
if __name__ == "__main__":
    ...
    ap.add_argument("--data", type=str, required=True, help="path to featurized_ft dir")
    ap.add_argument("--pretrained", type=str, required=True, help="path to backbone best_acc.pt")
    ap.add_argument("--outdir", type=str, default="./runs/epitope_ft")
    ...
    ap.add_argument("--d_model", type=int, default=256)  # 必须与预训练一致
    ap.add_argument("--lambda_hla", type=float, default=1.0)
    ap.add_argument("--lambda_seq", type=float, default=1.0)
    ...
    ap.add_argument("--dropout_tab", type=float, default=0.0)
    ap.add_argument("--dropout_seq", type=float, default=0.0)
    ap.add_argument("--slot_drop", type=float, default=0.0)
    ...
    ap.add_argument("--early_stop_patience", type=int, default=80)
